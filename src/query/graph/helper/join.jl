function process_arg!(
    ds, srcs, helpers_ex, ::Val{:innerjoin}, ex, index
)::Void
    # NOTE: Only equijoins are supported for now
    @assert ex.head == :call && ex.args[1] == :(==)
    lhs, rhs = ex.args[2], ex.args[3]
    f_ex1, arg_fields1 = build_f_ex!(Dict{Symbol, Set{Symbol}}(), srcs, lhs, index)
    f_ex2, arg_fields2 = build_f_ex!(Dict{Symbol, Set{Symbol}}(), srcs, rhs, index)
    push!(
        helpers_ex.args,
        Expr(
